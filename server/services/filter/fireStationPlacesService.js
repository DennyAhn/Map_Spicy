...
const fireStationPlacesService = {
  getFireStationPlacesData: async (lat, lng) => { // renamed function
    try {
      ...
      console.log(`소방시설 장소 요청 위치: lat=${lat}, lng=${lng}`);
      ...
      console.log('첫 번째 소방시설 데이터 예시:', data.documents[0]);
      ...
      const mappedData = data.documents.map(place => {
        ...
        const placeData = { 
          ...
          category: place.category_name || '소방서',    // 카테고리
      ...
      console.error('소방시설 데이터 가져오기 실패:', error);
      throw new Error('소방시설 데이터 가져오기 실패: ' + error.message);
    }
  }
};
